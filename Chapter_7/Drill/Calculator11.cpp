...
//Handle number and ( ) and variable
{
	...
	case Pow:{
		double base{1}; //base of exponent
		int exponent{1}; //exponent
		t = ts.get();
		if(t.kind == '('){
			d = expression();

		t = ts.get();
		if(t.kind != ',') error("pow: expected \' , \' after expression"); //after base ' , ' should come

		exponent = expression(); //take base from token stream 
